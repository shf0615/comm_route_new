...
通过自然的协作对话，帮助将想法转化为完整的行为规格和技术方案。
...
4. **展示设计** — 按各部分复杂度缩放篇幅，每部分完成后获取用户批准
5. **撰写规格文档** — 保存 .feature + -design.md 并提交
6. **规格自审** — 快速检查占位符、矛盾、歧义、范围（见下方）
7. **对抗性审查** — 派遣子agent独立审查，循环直到无严重问题
8. **用户审阅已写规格** — 请用户在继续之前审阅规格文件
9. **过渡到实现** — 调用 sp-writing-plans 技能来创建实现计划
...
digraph brainstorming {
    ...
    "调用 sp-writing-plans" [shape=doublecircle];
    ...
    "用户审阅规格?" -> "调用 sp-writing-plans" [label="批准"];
}
```

**终止状态是调用 sp-writing-plans。** 不要调用任何其他实现技能。头脑风暴之后唯一调用的技能是 sp-writing-plans。
...
2. **内部一致性：** 各部分之间是否有矛盾？架构是否与功能描述匹配？行为规格与技术方案是否一致？
