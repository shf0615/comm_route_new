...
4. **生成 BDD 场景** — 基于澄清结果，用 Gherkin 编写行为场景并保存为 `.feature` 文件，获取用户确认
5. **提出 2-3 种方案** — 包含权衡分析和你的推荐
6. **展示设计** — 按各部分复杂度缩放篇幅，每部分完成后获取用户批准
7. **撰写设计文档** — 保存到 `docs/superpowers/specs/YYYY-MM-DD-<主题>-design.md` 并提交
8. **规格自审** — 快速检查占位符、矛盾、歧义、范围（见下方）
9. **用户审阅已写规格** — 请用户在继续之前审阅规格文件
10. **过渡到实现** — 调用 writing-plans 技能来创建实现计划
...
- 基于澄清性问题获得的共识，用 Gherkin 语法编写行为场景
- 保存为 `docs/superpowers/specs/features/<主题>.feature` 文件
- 场景应覆盖核心用户行为和关键边界条件，不求穷举
- 使用 `Feature` / `Scenario` / `Given` / `When` / `Then` 标准结构
- 展示给用户确认——BDD 场景是后续设计和实现的验收依据
- 如果用户要求修改，迭代调整后重新确认
